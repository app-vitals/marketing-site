...
We patched around all of this for weeks — more than once. A doctor check that warned if two backends looked configured at once. Then actual enforcement, forcing a single backend to be selected instead of leaving it as a config value an agent could still route around out of habit. The confusion kept coming back anyway. It helped, but it was still the same shape of system — an interface with a hole in the middle where a real data layer should be, and whatever plugged into that hole inheriting all its host's limitations, whether that host was a JSON file with no concurrency model at all or an issue tracker never designed to be one.

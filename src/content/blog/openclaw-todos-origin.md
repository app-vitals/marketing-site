...
Billing was the first real gap I automated, because it was a genuinely manual mess before Bodhi. I was invoicing by hand, checking by hand whether a payment had actually landed, and — since it's just the two of us — Dave and I were even paying each other manually. Bodhi built the automation the same way it did everything else by then: code living in its own workspace, run on a cron, checked with lint and tests, bound by the same guardrails as everything else it touched. All of that moved onto Bodhi over the course of March, and the invoicing work specifically is what forced the whole pipeline to get reliable.

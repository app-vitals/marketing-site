...
excerpt: "Two problems ran under our pipeline's crons for months: commands doing more than one job, and work waiting for its turn instead of starting the moment it was ready. Dave tried to fix the second by fusing two commands into one — and broke the first one doing it. Here's how both problems, colliding again and again, finally got solved by the same fix."
readTime: "11 min read"
---

*Fifth post in the series on how Shipwright actually came together — [the first one](/blog/cloud-agent-review-origin/) covered the cloud review pipeline, [the second](/blog/openclaw-todos-origin/) covered OpenClaw and the original todo list, [the third](/blog/vitals-os-merge-origin/) covered the merge into Vitals OS, [the fourth](/blog/task-store-origin/) covered the task store. That post ended with a promise I want to pick back up directly: why the whole system polls instead of reacting to events, and why a command called `patch` had to exist before the rest of the pipeline could narrow back down to what it was actually supposed to do. This one's that story — two problems running underneath the whole thing, commands doing more than one job and work waiting for its turn instead of starting the moment it was ready, and how both of them turned four separately-scheduled crons into `shipwright-loop`, the single dispatcher that actually runs the pipeline today.*

`patch` and `deploy` shipped May 26th — clean, single-purpose commands, split off from a `review` that had been trying to do their jobs for them. Five days later, on May 31st, Dave shipped `review-patch.md`: one command that ran review and patch back to back in a single session, built explicitly to collapse the round trip between them — instead of review flagging something and waiting for patch's own cron to eventually notice, fix it, and hand back. Inside about a day it needed a fix: a skip condition it had shipped with turned out to be wrong and got reverted. By June 1st it had already been rewritten from scratch — not deleted, pared down into a thin orchestrator that just called `/shipwright:patch` and `/shipwright:review` as fresh sessions and looped between them until both came back with nothing to do. One command went from new, to broken, to rebuilt, in about 36 hours. And the version that replaced it kept running for another six weeks before anything better existed.

Two problems are tangled up in that one paragraph, and it's worth pulling them apart before going further. The first: for months, single commands kept trying to do more than one job — the original execute cron, then `review`, and now `review-patch` right after the split that was supposed to end that pattern. The second: work sat waiting for its cron's turn even when everything needed to act on it was already sitting there, ready. `review-patch` was Dave trying to fix the second problem. It walked straight into the first one instead — you can't buy back the wait between review and patch by gluing them into one session without also giving up the reason they'd just been split apart. Those two problems, colliding and re-colliding, are what this whole post is actually about. To see why `review-patch` was doomed the way it was, it helps to rewind to where both of them started.
...
The same day, each of the four got something the two-cron model never had: a precheck script — `check-dev-task.ts`, `check-review.ts`, `check-patch.ts`, `check-deploy.ts`, one per command. A full agent session is the expensive part of any of this — spinning up Claude, giving it tools, letting it read and reason — and most ticks of most crons find nothing worth doing. So before any of that starts, one of these small, plain scripts runs first, looks at the real state — the task queue, the open PRs — and answers a question with a candidate attached: is there anything to act on, and if so, which one. No agent involved, cheap enough to run every tick, testable like any other function. If the answer's no, the cron goes silent without ever paying for a session. If it's yes, that candidate is what the session that starts next actually works on.
...
That's not a rejection, though. The precheck scripts are already shaped like a trigger check on purpose — each one answers "is there real work, and what is it" in a fast, deterministic way, which is most of what an event handler would need to do anyway. If it's ever worth making the jump, the shape is already sitting there waiting to be wired to something other than a clock.

There's a wrinkle worth being honest about, too. Naming the candidate was supposed to be a side effect of the cost question — a detail, not the point. It stopped being a detail once the pipeline leaned on these scripts for more than a yes/no: naming the right candidate became the only thing standing between a tick and a decision, which is a lot more load-bearing than "cost gate" was ever supposed to mean. And it comes with a real gap: a static script can still miss something a live agent would catch just by looking. Whether that gap matters less under a real event model, where the trigger is already scoped to the one thing that changed — I don't know yet. Left open on purpose.
...
It's also the fix for the thing `review-patch` was actually trying to do, back at the top of this post. Review flags something on one tick; `patch` — still fully its own command, never touching review's job — can pick it up on the very next tick, a minute later, instead of waiting for its own cron slot to come back around. Same round trip Dave was chasing when he fused the two together. This time, nothing had to be glued to anything else to get it.
...
Two real problems ran under everything in this post: commands that kept doing more than one job, and work that had to wait for its turn instead of starting the moment it was ready. `review-patch` is the sharpest collision between them — Dave built it to fix the second problem and re-broke the first one doing it. The staggering saga is a smaller thread off the same root, not a third problem in its own right: solving the first problem meant four crons where there used to be two, hand-staggering couldn't keep up with four the way it did with two, and dropping it quietly cost five weeks of OOMs — which did real work making the case that the second problem needed an actual fix, not another patch. The fix that actually stuck didn't patch a schedule or a command. It changed what all four phases were, structurally — symmetric, single-purpose, and read by one arbiter every single minute instead of ticking on their own — which is what let it solve both of the real problems in the same move instead of trading one for the other. Every piece of Shipwright has to earn its place by pointing back to a specific problem it solved for a specific person — this one earned it twice over.

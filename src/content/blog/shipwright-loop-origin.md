...
Late May, we pulled those apart for real. `patch` became its own command — scan your own open PRs, fix what review flagged. `deploy` became its own command — check approval and CI, ship it. `review` got stripped down to one job: evaluate PRs and post findings, nothing else. Four clean responsibilities where there used to be two messy ones.

Each of the four also got its own precheck script that same week — `check-dev-task.ts`, `check-review.ts`, `check-patch.ts`, `check-deploy.ts`. A precheck is a small, deterministic script that runs before the real thing: it answers one question — is there actually work here — without spending the money it costs to spin up a full Claude session to go find out. That's what made the next decision affordable.

Four commands meant four crons where there used to be two — and hand-staggering four things doesn't work the way it did for two. So we didn't. All four went on identical thirty-minute schedules. That wasn't an oversight — staggering by hand doesn't scale past a couple of slots, and it felt like exactly the kind of hack we shouldn't be building permanent scheduling around. Better to run them together and deal with whatever that caused than keep hand-tuning cron offsets forever. It only worked because the precheck scripts made it cheap to ask "is there work" four times every thirty minutes instead of once.
...
That's not a rejection, though. The precheck scripts are already shaped like a trigger check on purpose — fast and deterministic, which is most of what an event handler would need to do anyway. If it's ever worth making the jump, the shape is already sitting there waiting to be wired to something other than a clock.

...
**Sequencing decision, 2026-09-21 (Dan):** rows 7-9 publish in the order **metrics (7) → HITL (9) → observability (8)**, not row-number order. Reasoning: metrics has the earliest real origin point (April, months before anything else in the backlog); HITL is born literally inside row 6's own 13-day window (2026-06-17, one day before the cutover), giving it the tightest continuity coming off post 6; observability runs last because its second wave (the Aug 18-31 audit-trail work) is the most recent material in the whole series to date, a natural closer. Caveat worth keeping in mind when drafting each one: these three threads overlap heavily in June/July (metrics' provider pivot and HITL's birth land the same week; observability's Sentry wave starts before HITL's PR-schema work) — the row order is the right *publishing* sequence, not a claim that the underlying work happened in three neat back-to-back eras.
...
- `2026-06-11` to `06-12` — migration tooling built: PR #1472 ("Bodhi→Shipwright
  ...
  phased rollout order stated in the commit: the first agent, then two more
  client-specific agents, then `warchild`, then the last agent (five total,
  canary-first) — **corrected 2026-09-21: three of those five agent names
  (the first one, plus the two client-specific ones after it) turned out to
  be real client identities, not pseudonyms, and have been scrubbed
  everywhere they appeared in this doc and in the post. Only `warchild`
  (an internal hub-agent persona, not client-tied — see the persona table
  below) and the last agent are confirmed safe to name directly. This is the
  second correction to this note — the first one still named the three
  scrubbed agents directly while explaining they'd been scrubbed, which was
  its own leak. Do not restate any of their names, or what they matched,
  or where — not here, not in a future pass.**
- `2026-06-12` — the first agent migrated, explicitly as the canary (the
  migration runbook referenced it by name directly as the canary lesson ahead
  of the next agent's rollout — another spot a real name got scrubbed from).
- `2026-06-16` — the third and fourth agents (one client-specific, one
  `warchild`) flipped onto Shipwright; secrets for the two already-migrated
  agents dropped.
...
client, on a completely different kind of project, so it was the easy thing
...
the project that a real alternative backend had to exist, not just a
stub — Dan recalled "maybe SQLite," confirmed against git: SQLite became the
*default* provider the same commit that introduced the seam (`#148`), with a
Postgres provider added the same day (`LDS-1.4`, `#155`).

**Why `TaskStoreProvider`, 2026-06-27 — this is the real spine of the post
(Dan):** not primarily a self-hosting or dependency-removal move — an
**accuracy** move. Before this, metrics were captured two ways, both
agent-executed steps embedded in the command playbooks themselves, not code:
1. **Manual `posthog_send.py` invocations** — `dev-task.md`, `review.md`, and
   `deploy.md` each had steps instructing the agent to shell out to
   `python3 posthog_send.py` at pipeline milestones (`task_started`,
   `pr_created`, `ci_result`, `task_complete`, `task_reviewed`,
   `task_deployed`, etc.).
2. **A session-JSONL snapshot** — `dev-task.md`'s Step 1 told the agent to
   snapshot its own Claude Code session transcript to calculate a token
   count/tally, carried forward through later steps.
Both were steps an agent could skip, mid-sequence, without anything
downstream noticing — exactly the "AI doing more work than we wanted it to"
failure mode Dan named in the framing question. Verified in git: the whole
sequence is `MME-1` through `MME-5`, all landing **2026-06-25 through
06-28** — `MME-1` adds token columns + a PATCH-complete endpoint on the admin
side (06-25), `MME-2`/`MME-3` wire the harness to report token usage and
task-execution data directly as part of normal request handling rather than
a separate agent-run step (06-26/27), `MME-4` is `TaskStoreProvider` itself
— answers all 16 metric kinds by querying the task store + admin APIs
directly (06-27) — and `MME-5` deletes `posthog_send.py`, every invocation
site across the three command files, and the Step 1 JSONL snapshot in one
commit (`f88eca947`, 06-28), explicitly "superseded by the task-store-backed
...
surfacing late in a very high-velocity repo (consistent with the
entropy-scan/docs-freshness patrols catching stale references elsewhere in
...
done, then it's best to write it in code and not leave it up to the agent.
If the agent has fewer steps it's less likely to skip things and do the
right thing." The `posthog_send.py`-deletion + JSONL-snapshot-deletion
commit is the single best piece of evidence for this thesis in the whole
series so far — a literal before/after of "agent-executed step" replaced by
...
2. **Where PostHog came from (4/3):** not a Shipwright decision — Dave had
   just implemented PostHog analytics for a client on a completely different
   kind of project, so it was the easy thing to reach for. State plainly that
   the task store didn't exist yet (earliest attempt seven weeks out) — this

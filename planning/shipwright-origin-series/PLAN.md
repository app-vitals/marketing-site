...
| 5 | System crons + the shipwright-loop dispatcher — patch and deploy commands added, the four-cron model with preCheck gates, and the loop mechanism itself: wake up every minute, check for work across dev-task/review/patch/deploy, execute. **Scoping note (2026-08-25):** per git, the patch/deploy/four-cron work happened in the *same week* as the task store rebuild (`SWC-1.1` patch command 2026-05-26, `SWD-2.1` deploy command 2026-05-26, `SWC-1.5` four-cron model 2026-05-27 — task store work spans 2026-05-25–28). Not clearly a separate era from post 4 chronologically. Dan raised (2026-08-25) whether this needs its own post or folds into post 4 — undecided, flagged for scoping when we get there. **Two more angles Dan flagged (2026-08-25), both belong in this post specifically, not row 4:** (1) the preCheck scripts exist to stop token waste — a cheap check runs before an expensive agent gets dispatched, specifically to confirm there's real work before spending on a full agent invocation, and to avoid the agent running when there's genuinely nothing to do; (2) a "why not events" angle — the loop polls (wake up every minute and check) rather than reacting to events (e.g. a task becoming ready triggering immediate dispatch), and the reasoning behind that choice is part of the story, not just the mechanics of how polling works. | Not planned yet | — |
